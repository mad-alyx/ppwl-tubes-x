...
export default function Timeline() {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex justify-center relative" onClick={() => { setOpenMenuId(null); }}>
      ...
      {replyingTo && (
        <div className="fixed inset-0 bg-[#242d34]/50 backdrop-blur-sm flex items-start justify-center z-50 p-4 pt-16">
          <div className="bg-black w-full max-w-[600px] rounded-2xl flex flex-col relative border border-gray-800">
            <div className="flex items-center px-4 py-2 border-b border-gray-800"><button onClick={() => { setReplyingTo(null); setReplyContent(""); setReplyImage(null); setActiveEmojiPicker(null); }} className="p-2 hover:bg-white/10 rounded-full transition"><X className="w-5 h-5"/></button></div>
      ...
      {/* === MODAL EDIT PROFIL (DIPERBARUI MEREPLIKA REFERENSI) === */}
      {isProfileEditOpen && (
      ...
      <div className="w-full max-w-[1265px] flex justify-between">
        ...
        <main className="flex-1 max-w-[600px] border-x border-gray-800 min-h-screen relative">
                ...
                {posts.length === 0 ? (
                  ...
                  <div key={post.id} onClick={() => loadPostDetailAndComments(post.id)} className="p-4 border-b border-gray-800 hover:bg-white/5 transition cursor-pointer">
                    <div className="flex gap-3">
                      <div className="w-10 h-10 bg-gray-700 rounded-full overflow-hidden shrink-0" onClick={(e)=>{e.stopPropagation(); switchView("profile")}}>{post.author?.avatarUrl ? <img src={post.author.avatarUrl} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-bold">{post.author?.name?.charAt(0) || "?"}</div>}</div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
          ...
          {currentView === "detail" && selectedPostDetail && (
            <>
              <div className="sticky top-0 bg-black/80 backdrop-blur z-10 border-b border-gray-800 p-4 flex items-center gap-6"><button onClick={() => switchView("home")} className="p-2 hover:bg-white/10 rounded-full transition"><ArrowLeft className="w-5 h-5" /></button><h2 className="text-xl font-bold">Postingan</h2></div>
              <div className="p-4 border-b border-gray-800">
                ...
                <div className="border-y border-gray-800 py-3 my-3 text-sm text-gray-500 flex gap-6"><span><strong className="text-white">{selectedPostDetail._count?.likes || 0}</strong> Suka</span><span><strong className="text-white">{selectedPostDetail._count?.comments || 0}</strong> Balasan</span></div>
