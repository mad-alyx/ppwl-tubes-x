...
export default function PostingForm({
  ...
  return (
    <div className="w-full max-w-[600px] flex flex-col min-h-screen">
      <div className="sticky top-0 bg-black/80 backdrop-blur z-10 border-b border-gray-800 p-4 flex items-center justify-between">
      ...
      <div className="flex-1 p-4 flex gap-3">
        ...
        <form onSubmit={(e) => handlePostSubmit(e, false)} className="flex flex-col flex-1 w-full">
          <div className="py-2"><textarea ref={textareaRef} className="w-full bg-transparent outline-none resize-none text-2xl placeholder-gray-500 overflow-hidden" placeholder="Apa yang sedang terjadi?!" rows={4} value={newPostContent} onChange={handleInputResize} /></div>
          {postImage && (
          ...
          <div className="flex justify-between items-center mt-4 relative">
            ...
            <div className="flex items-center gap-3">
              <span className={`text-sm ${charCount > MAX_CHARS ? "text-red-500 font-bold" : "text-gray-500"}`}>{charCount}/{MAX_CHARS}</span>
              <button type="submit" disabled={(!newPostContent.trim() && !postImage) || isSubmitting || charCount > MAX_CHARS} className="bg-[#1D9BF0] text-white font-bold py-2 px-6 rounded-full disabled:opacity-50 transition">Posting</button>
            </div>
          </div>
        </form>
      </div>
