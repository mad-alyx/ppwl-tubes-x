...
type Props = {
  ...
  composeImageInputRef: React.RefObject<HTMLInputElement | null> | null;
  ...
  onClose: () => void;
  userData: any;
  composeTextareaRef: React.RefObject<HTMLTextAreaElement | null> | null;
          ...
          <div className="flex items-center gap-4">
            ...
            {composeEditId && (
              <h2 className="font-bold text-lg">
                Edit {composeEditType === "post" ? "Postingan" : "Komentar"}
              </h2>
            )}
          </div>
          <button
            onClick={(e) => handlePostSubmit(e, true)}
            disabled={(!composeContent.trim() && !composeImage) || isSubmitting || composeCharCount > MAX_CHARS}
            className="bg-[#1D9BF0] text-white font-bold py-1.5 px-4 rounded-full disabled:opacity-50 transition"
          >
            {composeEditId ? "Simpan" : "Posting"}
          </button>
        </div>
        <div className="p-4 flex gap-3 overflow-y-auto max-h-[80vh]">
          ...
          <div className="flex flex-col flex-1 gap-3 w-full">
            <textarea
              ...
              onChange={(e) => handleInputResize(e, composeTextareaRef as React.RefObject<HTMLTextAreaElement | null>, setComposeContent)}
            />
            {composeImage && (
            ...
            <div className="flex justify-between items-center mt-1 border-t border-gray-800 pt-3 relative">
              <div className="flex text-[#1D9BF0] -ml-2">
                <input
                  ...
                  onChange={(e) => handleImageUpload(e, setComposeImage)}
                />
                <button
                  type="button"
                ...
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setActiveEmojiPicker(activeEmojiPicker === "compose" ? null : "compose")}
                    className="p-2 hover:bg-[#1D9BF0]/10 rounded-full transition"
                  >
              ...
              <div className="flex items-center gap-3 text-sm text-gray-500">
                <span className={composeCharCount > MAX_CHARS ? "text-red-500" : ""}>
                  {composeCharCount}/{MAX_CHARS}
                </span>
              </div>
            </div>
          </div>
