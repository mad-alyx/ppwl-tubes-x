...
type Props = {
  ...
  replyImageInputRef: React.RefObject<HTMLInputElement | null> | null;
      ...
      <div className="bg-black w-full max-w-[600px] rounded-2xl flex flex-col relative border border-gray-800">
        ...
        <div className="p-4 flex gap-3 relative max-h-[30vh] overflow-y-auto">
          <div className="flex flex-col items-center">
            <div className="w-10 h-10 bg-gray-700 rounded-full overflow-hidden shrink-0">
              {replyingTo.author?.avatarUrl
                ? <img src={replyingTo.author.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                : <div className="w-full h-full flex items-center justify-center font-bold">{replyingTo.author?.name?.charAt(0) || "?"}</div>
              }
            </div>
        ...
        <div className="p-4 flex gap-3 pt-0">
          ...
          <form onSubmit={handleReplySubmit} className="flex flex-col flex-1 gap-3">
            <textarea
              className="w-full bg-transparent outline-none resize-none text-xl placeholder-gray-500 pt-2"
            ...
            {replyImage && (
              <div className="relative w-fit">
                ...
                <button type="button" onClick={() => setReplyImage(null)} className="absolute top-2 right-2 bg-black/70 p-1.5 rounded-full hover:bg-black transition">
                  <X className="w-4 h-4"/>
                </button>
              </div>
            )}
            <div className="flex justify-between items-center border-t border-gray-800 pt-3 relative">
              <div className="flex text-[#1D9BF0]">
                ...
                />
                <button type="button" onClick={() => replyImageInputRef?.current?.click()} className="p-2 hover:bg-[#1D9BF0]/10 rounded-full transition">
                  <ImageIcon className="w-5 h-5" />
                </button>
                <div className="relative">
                  <button type="button" onClick={() => setActiveEmojiPicker(activeEmojiPicker === "reply" ? null : "reply")} className="p-2 hover:bg-[#1D9BF0]/10 rounded-full transition">
