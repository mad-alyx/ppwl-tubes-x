  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[1265px] flex justify-between">
        ...
        <header className="hidden md:flex w-[88px] xl:w-[275px] flex-col justify-between py-4 px-2 xl:px-4 h-screen sticky top-0">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-black border-t border-gray-800 flex justify-around items-center py-3 z-50">
          <div onClick={() => switchView("home")} className={`flex flex-col items-center cursor-pointer ${currentView === "home" || currentView === "detail" ? "text-white" : "text-gray-500"}`}>
            <Home className="w-6 h-6" />
          </div>
          <div onClick={() => switchView("notifications")} className={`flex flex-col items-center cursor-pointer ${currentView === "notifications" ? "text-white" : "text-gray-500"}`}>
            <Bell className="w-6 h-6" />
          </div>
          <div onClick={() => setIsComposeOpen(true)} className="bg-[#1D9BF0] rounded-full p-3 cursor-pointer">
            <svg viewBox="0 0 24 24" aria-hidden="true" className="w-5 h-5 fill-white">
              <path d="M23 3c-6.62-.1-10.38 2.421-13.05 6.095C7.945 12.827 2 21.3 2 21.3c-1.125 1.574-.63 3.65.688 4.238 1.15.518 2.627-.1 3.52-1.3l8.6-11.455C18.667 9.208 21.5 5.5 23 3zm-20.5 13.5c-.828 0-1.5.672-1.5 1.5s.672 1.5 1.5 1.5 1.5-.672 1.5-1.5-.672-1.5-1.5-1.5zm11.5-9.5c0 .828-.672 1.5-1.5 1.5s-1.5-.672-1.5-1.5.672-1.5 1.5-1.5 1.5.672 1.5 1.5z" />
            </svg>
          </div>
          <div onClick={() => switchView("profile")} className={`flex flex-col items-center cursor-pointer ${currentView === "profile" ? "text-white" : "text-gray-500"}`}>
            <User className="w-6 h-6" />
          </div>
          <div onClick={handleLogout} className="flex flex-col items-center cursor-pointer text-gray-500">
